...
babies = pd.read_csv('../../../data/processed/tok_baby_reviews.csv')

print("\nFiles read, converting tokens to lists.")
for frame in [movie, games, babies]:
    for col in ['summary_tokens', 'review_tokens']:
        frame[col] = frame[col].map(literal_eval)
...
babies_summary = preprocess.word_index(babies['summary_tokens'], vocab_map, unknown_int)
babies_review = preprocess.word_index(babies['review_tokens'], vocab_map, unknown_int) 
...
babies_summary = pad_sequences(sequences=babies_summary, 
                              maxlen=summary_len, 
                              dtype='int32', 
                              padding='pre', 
                              value=pad_mask_int)
babies_review = pad_sequences(sequences=babies_review, 
                             maxlen=review_len, 
                             dtype='int32', 
                             padding='pre',
                             truncating='pre',
...
movie_pred.to_csv('../../../data/processed/group5_movie_preds_phase3.txt', index=False)
...
games_pred.to_csv('../../../data/processed/group5_game_preds_phase3.txt', index=False)

babies_pred = thawn.predict([babies_summary, babies_review])
babies_pred = preprocess.undo_one_hot(babies_pred, label_list)
babies_pred = pd.DataFrame(babies_pred)
babies_pred.to_csv('../../../data/processed/group5_babies_preds_phase3.txt', index=False)
